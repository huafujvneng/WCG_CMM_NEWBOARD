...
//定义 遥测 查询命令 查询地址
uint16_t read_yc_buf0[3] = {0x0004, 0x2000 , 0x12 };

//定义 遥信 查询命令 查询地址
uint16_t read_yx_buf0[3] = {0x0002, 0x0000 ,0x30};

//遥控//断路器
uint16_t trans_ykdl_buf0[2] = {0x0005, 0x3000 };

//遥控压板
uint16_t trans_ykyb_buf0[2] = {0x0006, 0x4000 };
//读取遥脉
uint16_t trans_ym_buf0[3] = {0x0004, 0x2100 ,0x08};
//读取定值
uint16_t read_dz_buf0[3] = {0x0003, 0x1000 ,0x04};
//遥调定值
uint16_t trans_ytdz_buf0[3] = {0x0010, 0x1000 ,0x04};
...
/*返回值              null                                                      */ 
/********************************************************************************/
void datacopy(uint16_t *p1,uint16_t *p2,uint16_t num)
{
    uint16_t i = 0;
	 // uint8_t *p;
...
//按照字节复制
void datacopy_8(uint8_t *p1,uint8_t *p2,uint16_t num)
{
	  uint16_t i = 0;
...
/*返回值              true                                                      */ 
/********************************************************************************/
uint8_t arry_compa(uint16_t *p1,uint16_t *p2,uint16_t num)
{
	  uint16_t i;
...
/*    i                      返回发送数据长度                               */
/********************************************************************************/
uint8_t yk_dl_cmdmd600[5] = {0x05,0x30,0x00,0xff,0x00};
uint8_t yk_yb_cmdmd600[5] = {0x06,0x40,0x00,0x00,0x00};
uint8_t yk_time_cmdmd600[6] = {0x10,0x19,0x00,0x00,0x03,0x06};
uint8_t temptimedata[6];
uint8_t trans_yk_cmd_modbus(uint8_t uartportflg,uint8_t devindex,uint8_t *transbuf)
{
    uint16_t i=0;
	  uint8_t deviceaddr;
	  uint16_t num;
...
#define REMOTE_QHDZ_TYPE 5	
	...
	*/	
  	...
  	switch(uartportflg)
  	{
	  	case 0:
					  ...
					  if((yk_data_buf0[devindex][5] == 0x81)||(yk_data_buf0[devindex][5] == 0x82))//遥控选择
            {
					  ...
					  else if((yk_data_buf0[devindex][5] == 0x01)||(yk_data_buf0[devindex][5] == 0x02))//遥控执行
						{
							...
							transbuf[i++] = yk_dl_cmdmd600[0];
							transbuf[i++] = yk_dl_cmdmd600[1];							
							transbuf[i++] = yk_dl_cmdmd600[2] + ykinf*2 + ykdcc;// inf
							transbuf[i++] = yk_dl_cmdmd600[3];							
							transbuf[i++] = yk_dl_cmdmd600[4];		
							uart_status[uartportflg].uartdevice[devindex].uart_ykflg = 0;
              yk_data_buf0[devindex][0] = 0x55;							
							return i;
             ...
             return i;					
					...
					case REMOTE_YB_TYPE:
						  ...
						  transbuf[i++] = yk_yb_cmdmd600[0]; //0x06
						  transbuf[i++] = yk_yb_cmdmd600[1];					
						  transbuf[i++] = yk_yb_cmdmd600[2]+ykinf;
						  transbuf[i++] = yk_yb_cmdmd600[3];
						  transbuf[i++] = ykdcc;
						  uart_status[uartportflg].uartdevice[devindex].uart_ykflg = 0;
              yk_data_buf0[devindex][0] = 0x55;							
              return i;							
          ...
          case  REMOTE_TIME_TYPE://yk_time_cmdmd600[6] = {0x10,0x19,0x00,0x00,0x03,0x06};
						  transbuf[i++] = yk_data_buf0[devindex][3];
						  transbuf[i++] = yk_time_cmdmd600[0];
						  transbuf[i++] = yk_time_cmdmd600[1];
						  transbuf[i++] = yk_time_cmdmd600[2];
						  transbuf[i++] = yk_time_cmdmd600[3];
						  transbuf[i++] = yk_time_cmdmd600[4];
						  transbuf[i++] = yk_time_cmdmd600[5];
...
uint8_t devaddr_to_devindex(uint8_t uartportflg,uint8_t devaddr)
{
    uint8_t i;
	  for(i=0;i<DEVICEMAXNUM;i++)
	  {
		    if(uartpara[uartportflg].devaddr[i] == devaddr)
		    {
			      break;
	    	}
	  }
...
/*//定义 遥测 查询命令 查询地址
uint16_t read_yc_buf0[2] = {0x0004, 0x2000 };

//定义 遥信 查询命令 查询地址
uint16_t read_yx_buf0[2] = {0x0002, 0x0000 };
...
uint16_t trans_ykyb_buf0[2] = {0x0006, 0x4000 };
//读取遥脉
uint16_t trans_ym_buf0[2] = {0x0004, 0x2100 };
//读取定值
uint16_t read_dz_buf0[2] = {0x0003, 0x1000 };
//遥调定值
uint16_t trans_ytdz_buf0[2] = {0x0010, 0x1000 };*/
uint8_t read_yc_modbus(uint8_t uartportflg,uint8_t devindex,uint8_t *transbuf)
{
	  uint8_t i = 0;
	  transbuf[i++] = uartpara[uartportflg].devaddr[devindex];
	  transbuf[i++] = read_yc_buf0[0]&0xff;
	  transbuf[i++] = (read_yc_buf0[1]>>8)&0xff;
    transbuf[i++] = read_yc_buf0[1]&0xff;
	  transbuf[i++] = 0;
	  transbuf[i++] = read_yc_buf0[2]&0xff;
	  //transbuf[i++] = uartpara[uartportflg].deviceconf[devindex].ycnum;
	  return i;
}
//uint16_t trans_ym_buf0[3] = {0x0004, 0x2100 ,0x04};
uint8_t read_ym_modbus(uint8_t uartportflg,uint8_t devindex,uint8_t *transbuf)
{
	  uint8_t i = 0;
	  transbuf[i++] = uartpara[uartportflg].devaddr[devindex];
	  transbuf[i++] = trans_ym_buf0[0]&0xff;
	  transbuf[i++] = (trans_ym_buf0[1]>>8)&0xff;
    transbuf[i++] = trans_ym_buf0[1]&0xff;
	  transbuf[i++] = 0;
	  transbuf[i++] = trans_ym_buf0[2]&0xff;
	  //transbuf[i++] = uartpara[uartportflg].deviceconf[devindex].ycnum;
	  return i;
}

uint8_t read_yx_modbus(uint8_t uartportflg,uint8_t devindex,uint8_t *transbuf)
{
	  uint8_t i = 0;
	  transbuf[i++] = uartpara[uartportflg].devaddr[devindex];
	  transbuf[i++] = read_yx_buf0[0];	
	  transbuf[i++] = (read_yx_buf0[1]>>8)&0xff;
    transbuf[i++] = read_yx_buf0[1]&0xff;
	  transbuf[i++] = 0;
	  transbuf[i++] = read_yx_buf0[2]&0xff;
...
uint8_t  Deal_Comm_Proce_0(rt_device_t dev)//,uint8_t *tx_buf,uint8_t *rx_buf)//modbus WGB600
{
...
//    uint8_t tx_number;
    ...
    if(uart_status[uartportflg].uart_rtdbflg != 0)//有远方操作
    {
				    ...
				    if(uart_status[uartportflg].uartdevice[i].uart_ykflg == 1)
            {
			          ...
			          data_num = trans_yk_cmd_modbus(uartportflg,uart_status[uartportflg].reddevindex,uart->int_tx->tx_buffer);							
        ...
        if(uart_status[uartportflg].uart_redycyxflg == 0)// read yc data
				{
				    ...
				    data_num = read_yc_modbus(uartportflg,uart_status[uartportflg].reddevindex,uart->int_tx->tx_buffer);
				
				}
				else if(uart_status[uartportflg].uart_redycyxflg == 1)//read yx data
				{
				    data_num = read_yx_modbus(uartportflg,uart_status[uartportflg].reddevindex,uart->int_tx->tx_buffer);
				}
				else if(uart_status[uartportflg].uart_redycyxflg == 2)//read ym data
        {
					  data_num = read_ym_modbus(uartportflg,uart_status[uartportflg].reddevindex,uart->int_tx->tx_buffer);
				}
...
void Receive_Commdata_Proce_0(rt_device_t dev)//,uint8_t *tx_buf,uint8_t *rx_buf,uint8_t uartportflg)
{
	  uint8_t uartportflg;
	  uint16_t Check_Code=0;
