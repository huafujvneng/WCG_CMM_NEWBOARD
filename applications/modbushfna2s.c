...
//定义 遥测 查询命令 查询地址
uint16_t read_yc_buf_hfna[3] = { 0x0004, 0x2000 , 0x1e };

//定义 遥信 查询命令 查询地址
uint16_t read_yx_buf_hfna[3] = { 0x0002, 0x0000 ,0x78 };

//读取遥脉
uint16_t trans_ym_buf_hfna[3] = { 0x0004, 0x2100 ,0x10 };
//遥控//断路器
uint8_t yk_dl_cmd_hfna[5] = { 0x05,0x30,0x00,0xff,0x00 };
//遥控压板
uint8_t yk_yb_cmd_hfna[5] = { 0x06,0x40,0x00,0x00,0x00 };
//对时
uint8_t yk_time_cmd_hfna[6] = { 0x10,0x19,0x00,0x00,0x03,0x06 };
...
uint8_t trans_yk_cmd_modbus_2(uint8_t uartportflg, uint8_t devindex, uint8_t* transbuf)
{
	uint16_t i = 0;
	uint8_t deviceaddr;
	uint16_t num;
	...
	case 0:
		...
		case REMOTE_DL_TYPE:
			...
			else if ((yk_data_buf0[devindex][5] == 0x01) || (yk_data_buf0[devindex][5] == 0x02))//遥控执行
			{
				...
				transbuf[i++] = yk_dl_cmd_hfna[0];
				transbuf[i++] = yk_dl_cmd_hfna[1];
				transbuf[i++] = yk_dl_cmd_hfna[2] + ykinf * 2 + ykdcc;// inf
				transbuf[i++] = yk_dl_cmd_hfna[3];
				transbuf[i++] = yk_dl_cmd_hfna[4];
				uart_status[uartportflg].uartdevice[devindex].uart_ykflg = 0;
				yk_data_buf0[devindex][0] = 0x55;
				return i;
		...
		case REMOTE_YB_TYPE:
			...
			transbuf[i++] = yk_yb_cmd_hfna[0]; //0x06
			transbuf[i++] = yk_yb_cmd_hfna[1];
			transbuf[i++] = yk_yb_cmd_hfna[2] + ykinf;
			transbuf[i++] = yk_yb_cmd_hfna[3];
			transbuf[i++] = ykdcc;
			uart_status[uartportflg].uartdevice[devindex].uart_ykflg = 0;
			yk_data_buf0[devindex][0] = 0x55;
			return i;
		...
		case  REMOTE_TIME_TYPE://yk_time_cmd_hfna[6] = {0x10,0x19,0x00,0x00,0x03,0x06};
			transbuf[i++] = yk_data_buf0[devindex][3];
			transbuf[i++] = yk_time_cmd_hfna[0];
			transbuf[i++] = yk_time_cmd_hfna[1];
			transbuf[i++] = yk_time_cmd_hfna[2];
			transbuf[i++] = yk_time_cmd_hfna[3];
			transbuf[i++] = yk_time_cmd_hfna[4];
			transbuf[i++] = yk_time_cmd_hfna[5];
...
uint8_t read_yc_modbus_2(uint8_t uartportflg, uint8_t devindex, uint8_t* transbuf)
{
	uint8_t i = 0;
	transbuf[i++] = uartpara[uartportflg].devaddr[devindex];
	transbuf[i++] = read_yc_buf_hfna[0] & 0xff;
	transbuf[i++] = (read_yc_buf_hfna[1] >> 8) & 0xff;
	transbuf[i++] = read_yc_buf_hfna[1] & 0xff;
	transbuf[i++] = 0;
	transbuf[i++] = read_yc_buf_hfna[2] & 0xff;
	//transbuf[i++] = uartpara[uartportflg].deviceconf[devindex].ycnum;
	return i;
}
//uint16_t trans_ym_buf_hfna[3] = {0x0004, 0x2100 ,0x04};
uint8_t read_ym_modbus_2(uint8_t uartportflg, uint8_t devindex, uint8_t* transbuf)
{
	uint8_t i = 0;
	transbuf[i++] = uartpara[uartportflg].devaddr[devindex];
	transbuf[i++] = trans_ym_buf_hfna[0] & 0xff;
	transbuf[i++] = (trans_ym_buf_hfna[1] >> 8) & 0xff;
	transbuf[i++] = trans_ym_buf_hfna[1] & 0xff;
	transbuf[i++] = 0;
	transbuf[i++] = trans_ym_buf_hfna[2] & 0xff;
	//transbuf[i++] = uartpara[uartportflg].deviceconf[devindex].ycnum;
	return i;
}

uint8_t read_yx_modbus_2(uint8_t uartportflg, uint8_t devindex, uint8_t* transbuf)
{
	uint8_t i = 0;
	transbuf[i++] = uartpara[uartportflg].devaddr[devindex];
	transbuf[i++] = read_yx_buf_hfna[0];
	transbuf[i++] = (read_yx_buf_hfna[1] >> 8) & 0xff;
	transbuf[i++] = read_yx_buf_hfna[1] & 0xff;
	transbuf[i++] = 0;
	transbuf[i++] = read_yx_buf_hfna[2] & 0xff;
...
uint8_t  Deal_Comm_Proce_2(rt_device_t dev)//,uint8_t *tx_buf,uint8_t *rx_buf)//modbus WGB-611
{
	...
	if (uart_status[uartportflg].uart_rtdbflg != 0)//有远方操作
	{
		for (i = 0; i < DEVICEMAXNUM; i++)
		{
			if (uart_status[uartportflg].uartdevice[i].uart_ykflg == 1)
			{
				...
				data_num = trans_yk_cmd_modbus_2(uartportflg, uart_status[uartportflg].reddevindex, uart->int_tx->tx_buffer);
	...
	else
	{
		...
		if (uart_status[uartportflg].uart_redycyxflg == 0)// read yc data
		{
			...
			data_num = read_yc_modbus_2(uartportflg, uart_status[uartportflg].reddevindex, uart->int_tx->tx_buffer);

		}
		else if (uart_status[uartportflg].uart_redycyxflg == 1)//read yx data
		{
			data_num = read_yx_modbus_2(uartportflg, uart_status[uartportflg].reddevindex, uart->int_tx->tx_buffer);
		}
		else if (uart_status[uartportflg].uart_redycyxflg == 2)//read ym data
		{
			data_num = read_ym_modbus_2(uartportflg, uart_status[uartportflg].reddevindex, uart->int_tx->tx_buffer);
		}
...
void Receive_Commdata_Proce_2(rt_device_t dev)//,uint8_t *tx_buf,uint8_t *rx_buf,uint8_t uartportflg)
{
	uint8_t uartportflg;
	uint16_t Check_Code = 0;
