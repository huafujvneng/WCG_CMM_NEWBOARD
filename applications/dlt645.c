...
//读取定值
uint16_t read_dz_buf1[3] = {0x0003, 0x1000 ,0x04};
//遥调定值
uint16_t trans_ytdz_buf1[3] = {0x0010, 0x1000 ,0x04};
...
uint8_t read_yc_dlt645(uint8_t uartportflg,uint8_t devindex,uint8_t *transbuf)
{
	  uint8_t i = 0;
...
uint8_t read_ym_dlt645(uint8_t uartportflg,uint8_t devindex,uint8_t *transbuf)
{
	  uint8_t i = 0;
	  transbuf[i++] = uartpara[uartportflg].devaddr[devindex];
...
uint8_t  Deal_dlt645_Wgb(rt_device_t dev)//查询电度表
{
...
//    uint8_t tx_number;
    ...
    if(uart_status[uartportflg].uart_rtdbflg != 0)//有远方操作
    {
				    ...
				    if(uart_status[uartportflg].uartdevice[i].uart_ykflg == 1)
            {
			          ...
			          //data_num = trans_yk_cmd_dlt645(uartportflg,uart_status[uartportflg].reddevindex,uart->int_tx->tx_buffer);							
...
void Receive_dlt645_Wgb(rt_device_t dev)//,uint8_t *tx_buf,uint8_t *rx_buf,uint8_t uartportflg)
{
	  uint8_t uartportflg;
	  uint16_t Check_Code=0;
