...
void rt_hw_hard_fault_exception(struct stack_context* contex)
{
	...
	{
		rt_thread_t tid;
		tid = rt_current_thread;

		rt_kprintf("hard fault on thread: %-8.*s\n", RT_NAME_MAX, tid->name);
		if ((rt_uint32_t)tid->sp >= (rt_uint32_t)tid->stack_addr &&
			(rt_uint32_t)tid->sp <= (rt_uint32_t)tid->stack_addr + tid->stack_size)
		{
			rt_uint32_t *sp;

			sp = (rt_uint32_t*)tid->sp;
			while ((rt_uint32_t)sp <= ((rt_uint32_t)tid->stack_addr + tid->stack_size))
			{
